...
# 打入定时任务
if [ -z "$(cat /etc/crontab | grep pear_cron.sh | grep -v '#')" ]; then 
    echo "* * * * * root /root/pear/cron/pear_cron.sh -1m" >> /etc/crontab
    echo "*/5 * * * * root /root/pear/cron/pear_cron.sh -5m" >> /etc/crontab
    echo "*/30 * * * * root /root/pear/cron/pear_cron.sh -30m" >> /etc/crontab
    echo "15 4 * * * root /root/pear/cron/pear_cron.sh -1d" >> /etc/crontab
fi
